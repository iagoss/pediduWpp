<?php if($_GET['a'] == 1) {
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			if($insert) {
				$core->logger("O usuário adicionou uma nova categoria de módulo.", "acao");
...
<div class="box-content">
	...
	$form->createInput('Icone', 'text', 'icone');
...
<?php if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			if($update) {
				$core->logger("O usuário editou a categoria de módulo [#$id].", "acao");
...
<div class="box-content">
	...
	$form->createInput('Icone', 'text', 'icone', $ex['icone']);
...
<?php if($_GET['a'] == 3) {
	...
	if(count($ids) > 0) {
		...
		foreach ($ids as $id_atual) {
			...
			$core->logger("O usuário deletou a categoria de módulo [#$id_atual]", "acao");
		}
	} else {
		$delete_where['id'] = $id;
		$delete = $conn->prepare("UPDATE $mdl_tabela SET status = 'inativo' WHERE id = ? LIMIT 1");
		...
		$core->logger("O usuário deletou a categoria de módulo [#$id]", "acao");
	}
} ?>
