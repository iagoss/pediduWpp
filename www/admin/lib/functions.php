 ...
 */
$_configRevenda = $conn->prepare("SELECT * FROM resale_config");
$_configRevenda->execute();
$configRevenda = $_configRevenda->fetch();
...
function getSearchForm($cat = array()) {
    $height = count($cat) > 0 ? 'style="height: 80px"' : '';
    ...
    $retorno .= '<br>';
    if(count($cat) > 0){
        $retorno .= '<select class="form-input" name="categoria" id="categoria">';
        $retorno .= '<option value="all" selected="selected">Todas as categorias</option>';
        foreach($cat as $arr) {
           $retorno .= '<option value="'.$arr['id'].'">'.$arr['name'].'</option>'; 
        }
        $retorno .= '</select>';
    ...
    $retorno .= '<button type="submit" class="btn btn-info search-submit" '.$height.'>Pesquisar</button>';
    $retorno .= '</div>';
