...
if($_GET['a'] == 1) {
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			if($insert) {
				$core->logger("O usuário adicionou um novo aviso no painel.", "acao");
				$core->sendNtf("Novo aviso adicionado.", "warning");
...
<div class="box-content">
	...
	$form->mostraTitulo('Cargos que podem ler este aviso');
	$form->mostraAviso(well('Caso queira que todos os cargos leiam este aviso, não marque nenhum.'));
...
<?php if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			if($update) {
				$core->logger("O usuário editou o aviso [#$id].", "acao");
				$core->sendNtf("O aviso ($titulo) foi editado.", "warning");
...
<div class="box-content">
	...
	$sql3 = $conn->prepare("SELECT * FROM acp_avisos_lido WHERE id_aviso = ?");
	$sql3->bindValue(1, $id);
	$sql3->execute();
	...
	$form->mostraAviso(well("Usuários que marcaram este aviso como lido: <b>$leram</b>"));
...
<?php if($_GET['a'] == 3) {
	...
	if(count($ids) > 0) {
		...
		$delete2 = $conn->prepare("DELETE FROM acp_avisos_lido WHERE id_aviso = ?");
		...
		foreach ($ids as $id_atual) {
			...
			$core->logger("O usuário deletou o aviso [#$id_atual]", "acao");
		}
	} else {
		$delete_where['id'] = $id;
		$delete = $conn->prepare("UPDATE $mdl_tabela SET status = 'inativo' WHERE id = ? LIMIT 1");
		...
		$delete_where['id_aviso'] = $id;
		$delete = $sqlActions->delete("acp_avisos_lido", $delete_where);

		$core->logger("O usuário deletou o aviso [#$id]", "acao");
	}
} ?>
