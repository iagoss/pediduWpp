<?php if($_GET['a'] == 1) {
    if($_POST['form'] == 'form') {
        ...
        $id_user = $core->clear($_POST['id_user']);
        $prosseguir = true;

        if(empty($name) || empty($id_user)) {
    ...
    <div class="box-content">
        ...
        $categorias = array();
        $sql = $conn->query("SELECT * FROM acp_usuarios ORDER BY id ASC");
        while($sql2 = $sql->fetch()) {
            $atual = array("label" => $core->clear($sql2['nick']), "value" => $sql2['id']);
            $categorias[] = $atual;
        }

        $form->createInput('Nome', 'text', 'name');
        $form->createSelect('Selecione o usuário do entregador', 'id_user', $categorias);
