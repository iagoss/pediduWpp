...
if (isset($_POST['id'])) {
    ...
    while ($row = $query->fetch()) :
        ...
        $orderHtml .= "<div><center>Data {$dataArray['order']['date']}</center></div><br/>";
        ...
        if ($row['delivery_option'] == 1) {
            $orderHtml .= "<div><b>RUA: {$dataArray['client']['address_name']}</b></div>";
            $orderHtml .= "<div><b>NUMERO: {$dataArray['client']['address_number']}</b></div>";
            $orderHtml .= "<div><b>REGIAO: {$dataArray['client']['address_region']}</b></div>";
            $orderHtml .= "<div><b>REFERENCIA: {$dataArray['client']['address_reference']}</b></div>";
        ...
        foreach ($purchases as $purchase) {
            ...
            $orderHtml .= ($purchase->product_optional ? "OBS: $purchase->product_optional </br>" : "") . "</br>";
        ...
        $orderHtml .= "<div><b>Forma de pagamento: {$dataArray['order']['payment_methodo']} </b></div>";
