...
class Form {
	...
	public function createInputHidden() {
	    ...
	    <div class="form-group">
	        <input type="hidden" name="empty" value="empty_input">
	    </div>
		<br>';
	}
	...
	public function createInput($label, $tipo, $nome, $value = '', $classes = '', $disabled = '', $info = '') {
		...
		if($tipo == 'number'){
            ...
            <input class="form-input" step="any" type="'.$tipo.'" name="'.$nome.'" id="'.$nome.'" placeholder="'.$label.'" value="'.$value.'"'.$disabled.'>
            <br>';
        }else{
            $this->form_inputs .= '<div class="form-group'.$classes.'">
            <label class="form-label" for="'.$nome.'">'.$label.'</label>
    ...
    public function createBlankSpace() {
        $this->form_inputs .= '<div style="width: 100%; height: 5px; margin: 15px 0; background: #e4e4e4; border-radius: 5px;"></div>';
    }
