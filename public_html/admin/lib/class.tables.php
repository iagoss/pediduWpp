...
class Table {
	 ...
	 */
	public function __construct($classes = '', $checks = true, $allAccess = null, $iscontaspag = false) {
		if(!empty($classes)) {
			$classes = ' ' . $classes;
		}
	 ...
	 * @param  array $nomes
	 */
	public function insertBody($nomes, $status = null, $ignoreLink = false, $data = false) {
		$p = $_GET['p'];
		$id = $nomes[0];
		$link = "?p=$p&a=2&id=$id";
		...
		$this->table_body .= $data ? '<tr data-id='.$id.'>' : '<tr>';
