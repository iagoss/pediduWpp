...
$(document).ready(function() {
    ...
    //CHECK ADDITIONAIS (ORDENAR)
    function ordenar(el, time){
        var lastChecked = $('.check-add input[type="checkbox"]:checked').not(el).last().closest('.check-add');
        var firstRegister = $('.check-add input[type="checkbox"]').first().closest('.check-add');
        var whatToMove = $(el).closest('.check-add');
        $(whatToMove).animate({
         width: 'toggle'   
        },time, function(){
            if(lastChecked.length > 0) {
                $(this).insertAfter(lastChecked).animate({width:'toggle'}, time);
            } else {
                $(this).insertBefore(firstRegister).animate({width:'toggle'}, time);
            }
        });
    }
    ...
    function virgulas(){
        var order = [];
        $('.check-add input[type="checkbox"]:checked').each(function(){
           order.push($(this).attr('name').split('c-')[1]); 
        });
        $('#additionals').val(order.join(',')+',');
    }
    //COLOCAR INPUTS DENTRO DE DIV
    $('<div class="addit"></div>').insertBefore($('.check-add:first'));
    $('.check-add').each(function(){
        $(this).appendTo('.addit');
    });
    
    //ORDENAR POR SORTABLE
    $( ".addit" ).sortable({
    ...
    //ESCONDER INPUT#ADDITINALS
    $('input#additionals').hide();
    
    //ORDENAR AO CLICAR
    $('.check-add input[type="checkbox"]').on('change', function(){
        ordenar(this, 100);
        virgulas();
    });
    
    //ORDENAR TODOS (AO INICIAR TELA)
    if($('#additionals').val()){
        var ordem = $('#additionals').val().slice(0,-1).split(',');
        $.each(ordem.reverse(), function(){
           ordenar($('input#c-'+this).closest('.check-add'), 0);
        });
        $.each(ordem.reverse(), function(){
           $('input#c-'+this).attr('checked', 'checked').prop('checked', true);
        });
    }
    // CHECK DIAS
    var checkado = $('.check-dias.check-all input[name="w-all"]').prop('checked');
    if(checkado) {
        $('.check-dias:not(".check-all")').hide();
    }
    // MARCAR/DESMARCAR TODOS OS DIAS (PRODUTOS)
    $('.check-dias input[type="checkbox"]').on('click', function(){
        var dia = $(this).attr('name').split('w-')[1];
        console.log(dia);
        if(dia == 'all'){
            if($(this).is(':checked') == true){
                $('.check-dias input[type="checkbox"]').attr('checked', 'checked').prop('checked', true);
                $('.check-dias').hide();
                $('.check-dias.check-all').css('display', 'flex');
            } else {
                $('.check-dias input[type="checkbox"]').prop('checked', false);
                $('.check-dias').css('display', 'flex');
            }
        } else {
            if($(this).is(':checked') == false){
                $('.check-dias input[name="w-all"]').prop('checked', false);
            }
        }
    })
    //
